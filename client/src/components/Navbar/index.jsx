...
export default function Navbar() {
  ...
  return (
    <nav className="h-16 bg-white px-4 md:px-6 flex items-center gap-1 md:gap-0 justify-between shadow-md md:text-[18px]">
      {isMobile && (
        ...
        <>
          ...
          <div id="mobileNav" className="flex items-center gap-3 ml-[50px]">
            {navItems.map((item, index) => (
              <button key={index} onClick={navigate.bind(this, item.href)}>
                {item.name}
              </button>
            ))}
          </div>
...
function AuthNav({ isMobile, user, navigate }) {
    ...
    <>
      {!user && !isMobile && (
        <div id="userNav" className="flex gap-3">
          <button onClick={signIn}>Inicia sesión</button>
